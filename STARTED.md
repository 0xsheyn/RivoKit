...
npm test                    # 488 tests / 28 files — needs no credentials at all
```
...
| Command | Does |
|---|---|
| `npm test` | vitest — 488 green / 28 files, no credentials |
...
| An order stuck at `settlement_pending` | Captured but not converted; the funds are safe with the receiver as USDC. Read `order.failureReason`, then call `retrySettlement(orderId)` — **not** `release()`, which would capture a second time. The marketplace Host panel exposes it as **Retry settlement**. |
| An order stuck at `funding_pending` | Neither of the other sweeps can see it — one repairs a payout row, the other a `cpn_payments` row. Run `scripts/live-order-reconcile.mjs`: it asks the escrow whether it holds the authorized amount and promotes the order if it does. A `refund_pending` order is reported, never closed — escrow state says nothing about the origin-chain leg. |
| "Which failure path can I even rehearse right now?" | `scripts/check-resilience.mjs`. Read-only: it names the order id for every reachable scenario, the tool that closes it, and which listings the FX market can currently settle at all. |
| Waiting an hour to see an order expire | Don't. `scripts/demo-expire.mjs <orderId>` ages an **unpaid** order's `preApprovalExpiry` by a minute, and prints the command that puts it back. It refuses anything that has touched the chain: that timestamp is hashed into the payment's on-chain identity. |
