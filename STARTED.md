...
| A Circle wallet's signature is rejected by USDC or CPN | It is almost certainly `accountType: "SCA"`, whose signature is ERC-1271 — validated by calling the account contract, and recovering to nothing. Both ERC-3009 and CPN's settlement contract recover an ECDSA signer. The type is fixed at creation; make a new wallet with `accountType: "EOA"`. Roles that only execute contracts (operator, merchant) are unaffected. |
