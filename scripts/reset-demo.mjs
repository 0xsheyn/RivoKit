...
if (!url || !key) {
  console.error("GAGAL: NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SECRET_KEY tidak ada di .env.local");
  process.exit(1);
}
...
console.log(`sebelum → orders: ${await count("orders")}, payments: ${await count("payments")}, events: ${await count("events")}`);
...
console.log(`sesudah → orders: ${await count("orders")}, payments: ${await count("payments")}, events: ${await count("events")}`);
console.log("Riwayat demo dibersihkan.");
